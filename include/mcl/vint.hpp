...
class FixedBuffer {
 ...
 public:
  FixedBuffer() : size_(0) {}
  FixedBuffer(const FixedBuffer& rhs) { operator=(rhs); }
...
#pragma GCC diagnostic pop
  ...
  const Unit& operator[](size_t n) const {
    verify(n);
    return v_[n];
  }
...
class Vint {
 public:
  ...
  static const int invalidVar =
      -2147483647 - 1;  // abs(invalidVar) is not defined
 private:
  Buffer buf_;
  size_t size_;
  bool isNeg_;
  ...
  static void uadd(Vint& z, const Buffer& x, size_t xn, const Buffer& y,
                   size_t yn) {
    const Unit* px = &x[0];
    const Unit* py = &y[0];
    if (yn > xn) {
    ...
    assert(xn >= yn);
    bool b;
    // &x[0] and &y[0] will not change if z == x or z == y because they are
    // FixedBuffer
    z.buf_.alloc(&b, xn + 1);
    assert(b);
    if (!b) {
  ...
  static void usub(Vint& z, const Buffer& x, size_t xn, const Buffer& y,
                   size_t yn) {
    assert(xn >= yn);
    bool b;
    z.buf_.alloc(&b, xn);
  ...
  static void _add(Vint& z, const Vint& x, bool xNeg, const Vint& y,
                   bool yNeg) {
    if ((xNeg ^ yNeg) == 0) {
      // same sign
  ...
  /**
          ...
          @param r [out] x % y
  */
  static void udiv(Vint* q, Vint& r, const Buffer& x, size_t xn,
                   const Buffer& y, size_t yn) {
    assert(q != &r);
    if (xn < yn) {
      r.buf_ = x;
      r.trim(xn);
  ...
  */
  ...
  static void powT(Vint& z, const Vint& x, const Unit* y, size_t n,
                   const Mul& mul, const Sqr& sqr) {
    while (n > 0) {
      if (y[n - 1]) break;
      n--;
    }
 ...
 public:
  Vint(int x = 0) : size_(0) { *this = x; }
  Vint(Unit x) : size_(0) { *this = x; }
  Vint(const Vint& rhs)
      : buf_(rhs.buf_), size_(rhs.size_), isNeg_(rhs.isNeg_) {}
  Vint& operator=(int x) {
    assert(x != invalidVar);
    isNeg_ = x < 0;
    bool b;
  ...
  void dump(const char* msg = "") const { bint::dump(&buf_[0], size_, msg); }
  /*
          set positive value
  ...
  */
  ...
  void save(bool* pb, OutputStream& os, int base = 10) const {
    ...
    char buf[1024];
    size_t n =
        mcl::fp::arrayToStr(buf, sizeof(buf), &buf_[0], size_, base, false);
    if (n == 0) {
      *pb = false;
      return;
    }
  ...
  */
  ...
  static void sqr(Vint& y, const Vint& x) { mul(y, x, x); }
  static void addu1(Vint& z, const Vint& x, Unit y) { _addu1(z, x, y, false); }
  static void subu1(Vint& z, const Vint& x, Unit y) { _addu1(z, x, y, true); }
  static void mulu1(Vint& z, const Vint& x, Unit y) {
    size_t xn = x.size();
    size_t zn = xn + 1;
    bool b;
    std::cout << "go z.buf_.alloc\n";
    z.buf_.alloc(&b, zn);
    if (!b) {
      std::cout << "b is null\n";
    }
  ...
  static void divu1(Vint& q, const Vint& x, Unit y) { udivModu1(&q, x, y); }
  ...
  */
  ...
  static void mod(Vint& r, const Vint& x, const Vint& y) { divMod(0, r, x, y); }
  static void divs1(Vint& q, const Vint& x, int y) { divMods1(&q, x, y); }
  ...
  */
  ...
  /*
           1 if m is quadratic residue modulo n (i.e., there exists an x s.t.
     x^2 = m mod n) 0 if m = 0 mod n -1 otherwise
  ...
  */
...
#ifndef CYBOZU_DONT_USE_STRING
  explicit Vint(const std::string& str) : size_(0) { setStr(str); }
  ...
  friend bool operator<(const Vint& x, const Vint& y) {
  ...
  friend bool operator<(const Vint& x, uint32_t y) {
    return compareu1(x, y) < 0;
  }
  ...
  Vint& operator&=(Unit rhs) {
    andBitu1(*this, *this, rhs);
    return *this;
  }
  Vint& operator|=(Unit rhs) {
    orBitu1(*this, *this, rhs);
    return *this;
  }
  ...
  friend Vint operator+(const Vint& a, int b) {
    Vint c;
    adds1(c, a, b);
    return c;
  }
  ...
  friend Vint operator+(const Vint& a, Unit b) {
    Vint c;
    addu1(c, a, b);
    return c;
  }
